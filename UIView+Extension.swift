...
enum GoxViewCornerType {
    case icon
    case custom(topLeft: CGFloat = 0, topRight: CGFloat = 0, bottomLeft: CGFloat = 0, bottomRight: CGFloat = 0)
    case rounded
}
...
extension UIView{
    ...
    /// Round corner of a view with a given type corner
    ///  This round corner use CAShapeLayer mask
    /// - Parameter type: view corner type
    func roundCorners(type: GoxViewCornerType) {
        
        let shape = CAShapeLayer()
        let topLeftRadius: CGSize
        let topRightRadius: CGSize
        let bottomLeftRadius: CGSize
        let bottomRightRadius: CGSize
        
        switch type {
        case .icon:
            topLeftRadius = CGSize(width: 10, height: 10)
            topRightRadius = CGSize(width: 3, height: 3)
            bottomLeftRadius = CGSize(width: 3, height: 3)
            bottomRightRadius = CGSize(width: 3, height: 3)
        case .custom(topLeft: let topLeft, topRight: let topRight, bottomLeft: let bottomLeft, bottomRight: let bottomRight):
        ...
        case .rounded:
            topLeftRadius = CGSize(width: self.frame.height / 2, height: self.frame.height / 2)
            topRightRadius = CGSize(width: self.frame.height / 2, height: self.frame.height / 2)
            bottomLeftRadius = CGSize(width: self.frame.height / 2, height: self.frame.height / 2)
            bottomRightRadius = CGSize(width: self.frame.height / 2, height: self.frame.height / 2)
        }
    ...
    /// this impelmntation use view layer
    /// - Parameters:
    ///   - color:  color of the shadow,  default is black
    ///   - opacity: opacity of the shadow,  default is 1
    ///   - offset: offset of the shadow,  deafult is zero
    ///   - radius: the shadow radius, default is zero
    func setShadowWithColor(color: UIColor = .black, opacity: Float = 1.0, offset: CGSize = .zero, radius: CGFloat = 0) {
    ...
    /// Add a preset black shadow to the view
    /// This implementaton use CAShapeLayer
    /// Jump to code to see the detail
    func addShadow(){
    ...
    /// - Parameters:
    ///   - value: true for hidding and false to showing
    ...
    /// Set the fade fo the view
    /// - Parameters:
    ///   - style: bototm top right left vertical horizontal, default is bottom
    ///   - percentage: alpha percentage, default is 0.07
...
/*
===============================================
UIView extension for an animation
Source: https://github.com/keithito/SimpleAnimation
